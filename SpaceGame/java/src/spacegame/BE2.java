             ...
             for(int x = 0; x<theUniverse.getSector(a , b).ships.size(); x++)
               {
                 if(x+1<=theUniverse.getSector(a , b).ships.size())    
                  {
                   if(Roster.theRoster.get(theUniverse.getSector(a , b).ships.get(x).getPlayerNum()).getShipStats()
                      .getDefense()*randy.nextInt(4)*theUniverse.getSector(a , b).ships.get(x).getNumShips() <
                     Roster.theRoster.get(theUniverse.getSector(a , b).ships.get(x+1).getPlayerNum()).getShipStats()
                      .getAttack()*randy.nextInt(3)*theUniverse.getSector(a , b).ships.get(x).getNumShips()  )
                     {    
