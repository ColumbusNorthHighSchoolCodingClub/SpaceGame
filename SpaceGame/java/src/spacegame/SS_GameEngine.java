...
public class SS_GameEngine 
{
    ...
    public static String getPackedUniverse() { return theUniverse.pack(); }   
    public static String getPackedRoster() { return Roster.pack(); }
    public static String getPackedMarket() { return theMarket.pack(); }
    
    //================================================================
       
    public static void processInbox() 
    {
        for(int q=0;q<inbox.size();q++)
        {   //Loop through the entire inbox... 
            ...
            if(theUniverse.getSector(a ,b).hasPlanet() == true) //needs an accessor....
               {
                   ...
                   double fuelZ = 0;
                    materialsZ += (Roster.theRoster.get(player).getTechnology().getMineSpeed()*.05 +.2)*
                    theUniverse.getSector(a, b).planet.getMaterialCapacity();
                   
                    fuelZ += (Roster.theRoster.get(player).getTechnology().getFuelSpeed()*.05 +.2)*
                    theUniverse.getSector(a, b).planet.getFuelCapacity();
    ...
    {   
        ...
        do{
           ...
           counter++;
            ...
            while(counter<max && 
                (theUniverse.getSector(xOne , yOne).hasPlanet() && theUniverse.getSector(xOne ,yOne).planet.getControlledBy() != -1) 
                || theUniverse.getSector(xOne , yOne).hasPlanet() == false) ;
